...
otu.rarecurve = rarecurve(otu.rare, step = 50, label = T)

sp <- tax_glom(ps1, taxrank = "Species")

otu.rare = otu_table(sp)
otu.rare = as.data.frame(t(otu.rare))
sample_names = rownames(otu.rare)
otu.rarecurve = rarecurve(otu.rare, step = 50, label = T)
...
t <- read.table("NanoPlots/summary.tsv", sep = "\t", header = T)
raw <- select(t, BarCode, RawReads)
colnames(raw) <- c("Sample", "sum") 
raw$Rank <- c("Crudo")
raw <- select(raw, Sample, Rank, sum)

filt <- select(t, BarCode, Filtered)
colnames(filt) <- c("Sample", "sum") 
filt$Rank <- c("Filtrado")
filt <- select(filt, Sample, Rank, sum)

tab <- as.data.frame(rbind(a, b, c, d, e, f, g, raw, filt))
tab$Rank <- gsub("Kingdom", "Reino", tab$Rank)
tab$Rank <- gsub("Phylum", "Filo", tab$Rank)
tab$Rank <- gsub("Class", "Clase", tab$Rank)
tab$Rank <- gsub("Order", "Orden", tab$Rank)
...
level_order <- c("Crudo", "Filtrado", "Reino", "Filo", "Clase", "Orden", "Familia", "Género", "Especie") 
